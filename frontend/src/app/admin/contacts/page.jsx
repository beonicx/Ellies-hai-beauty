  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">Contact form submissions and enquiries</p>
      </div>

      <div className="flex flex-wrap gap-2 mb-4">
        {STATUS_FILTERS.map(s => (
          <button key={s} onClick={() => setStatusFilter(s)}
            className={`px-4 py-1.5 rounded-full text-xs font-medium capitalize transition-colors ${statusFilter === s ? "bg-[#C9956B] text-white" : "bg-white border border-gray-200 text-gray-600 hover:border-[#C9956B]"}`}>
            {s}
          </button>
        ))}
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                  ...
                  <div className="flex gap-1">
                    {msg.status !== "archived" && (
                      <button onClick={e => { e.stopPropagation(); handleArchive(msg._id); }}
                        className="p-1.5 text-gray-400 hover:text-yellow-600 rounded" title="Archive">
                        <Archive size={14} />
                      </button>
                    )}
                  ...
                  <div className="px-4 lg:px-6 pb-4 bg-gray-50 border-t border-gray-100">
                    <div className="py-4 space-y-3 text-sm">
                      <div className="flex flex-wrap gap-x-6 gap-y-1 text-gray-500">
                        <span>Email: <span className="text-gray-700">{msg.email}</span></span>
                        {msg.phone && <span>Phone: <span className="text-gray-700">{msg.phone}</span></span>}
                      </div>
                      <div>
        ...
        {pagination.pages > 1 && (
          <div className="px-6 py-3 border-t border-gray-100 flex items-center justify-between text-sm">
            <span className="text-gray-500">{pagination.total} messages</span>
