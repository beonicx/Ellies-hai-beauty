    ...
    <div>
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage inventory and retail products</p>
        </div>
        <button
          onClick={openCreate}
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 mb-4 flex flex-wrap gap-3 items-center">
        ...
        <button
          ...
          className={`px-3 py-2 text-sm border rounded-lg transition-colors ${lowStock ? "bg-red-50 border-red-200 text-red-700" : "border-gray-200 text-gray-600 hover:bg-gray-50"}`}
        >
          Low Stock Only
        </button>
      </div>
                ...
                {items.map((prod) => {
                  ...
                  return (
                    <tr key={prod._id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                      ...
                      <td className="px-5 py-3 text-gray-700">{prod.category}</td>
                      <td className="px-5 py-3 text-gray-700">₹{prod.price}</td>
                      <td className="px-5 py-3 text-gray-700">{prod.stock}</td>
                      <td className="px-5 py-3">
                        {!prod.isActive ? (
                      ...
                      <td className="px-5 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button onClick={() => openEdit(prod)} className="p-1.5 rounded hover:bg-gray-100 text-gray-500 hover:text-gray-700"><Edit3 size={14} /></button>
                          <button onClick={() => handleDelete(prod._id)} className="p-1.5 rounded hover:bg-red-50 text-gray-400 hover:text-red-600"><Trash2 size={14} /></button>
                        </div>
                      </td>
                    </tr>
                  );
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Cost Price (₹)</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Low Stock Threshold</label>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-sm">
            <div className="px-6 py-4 border-b border-gray-100">
              ...
              <p className="text-xs text-gray-500 mt-0.5">{stockModal.name} — current: {stockModal.stock}</p>
            </div>
            <form onSubmit={handleStockUpdate} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Adjustment (+ or −)</label>
                <input
                  type="number"
