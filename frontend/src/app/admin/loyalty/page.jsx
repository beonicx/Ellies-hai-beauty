...
import { loyalty } from "../../../lib/adminApi";
...
export default function AdminLoyaltyPage() {
  ...
  const loadLeaderboard = useCallback(async () => {
    try {
      ...
      const res = await loyalty.getLeaderboard({ limit: 20 });
  ...
  const loadTransactions = useCallback(async () => {
    try {
      setLoadingTxns(true);
      const res = await loyalty.getTransactions({ page, limit: 20 });
      if (res.success) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (modal === "add") {
        await loyalty.addPoints(data);
      } else {
        await loyalty.redeemPoints(data);
      }
      setModal(null);
    ...
    <div>
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage loyalty points and rewards</p>
        </div>
      ...
      <div className="mb-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          {loadingBoard ? (
            ...
            <div className="p-8 text-center"><p className="text-gray-500 text-sm">No loyalty data yet</p></div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 text-left">
                  <tr>
                    <th className="px-4 lg:px-6 py-3 text-xs font-medium text-gray-500 uppercase w-12">#</th>
                    <th className="px-4 py-3 text-xs font-medium text-gray-500 uppercase">Name</th>
                    <th className="px-4 py-3 text-xs font-medium text-gray-500 uppercase text-right">Points</th>
                    <th className="px-4 py-3 text-xs font-medium text-gray-500 uppercase text-right hidden sm:table-cell">Total Spent</th>
                    <th className="px-4 py-3 text-xs font-medium text-gray-500 uppercase text-right hidden sm:table-cell">Visits</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {leaderboard.map((user, i) => (
                    <tr key={user._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-right font-semibold text-[#C9956B]">{user.loyaltyPoints}</td>
                      <td className="px-4 py-3 text-right text-gray-600 hidden sm:table-cell">₹{user.totalSpent || 0}</td>
                      <td className="px-4 py-3 text-right text-gray-600 hidden sm:table-cell">{user.visitCount || 0}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          {loadingTxns ? (
            ...
            <div className="p-8 text-center"><p className="text-gray-500 text-sm">No transactions yet</p></div>
          ) : (
            <>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 text-left">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-xs font-medium text-gray-500 uppercase text-right">Points</th>
                      <th className="px-4 py-3 text-xs font-medium text-gray-500 uppercase hidden md:table-cell">Description</th>
                      <th className="px-4 py-3 text-xs font-medium text-gray-500 uppercase text-right hidden sm:table-cell">Balance</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {transactions.map(t => (
                      <tr key={t._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-gray-600 max-w-[200px] truncate hidden md:table-cell">{t.description || "—"}</td>
                        <td className="px-4 py-3 text-right text-gray-500 hidden sm:table-cell">{t.balanceAfter}</td>
              ...
              {pagination.pages > 1 && (
                <div className="px-6 py-3 border-t border-gray-100 flex items-center justify-between text-sm">
                  <span className="text-gray-500">{pagination.total} transactions</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setModal(null)}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h2 className="text-lg font-semibold text-gray-900">{modal === "add" ? "Add Points" : "Redeem Points"}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Points *</label>
              ...
              <button type="submit" disabled={saving}
                ...
                {saving ? "Processing..." : modal === "add" ? "Add Points" : "Redeem Points"}
              </button>
            </form>
          </div>
        </div>
